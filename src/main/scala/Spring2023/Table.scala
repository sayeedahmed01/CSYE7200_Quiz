...
case class Table[T](xs: List[T]) {
   ...
   */
  def lensFilter[P](p: P => Boolean)(lens: T => P): Table[T] = /** SOLUTION END */ Table(xs.map(x => p(lens(x))))
   ...
   */
  def sample(n: Int)(implicit r: Random): Table[T] = /** SOLUTION END */ filter(_ => r.nextInt(n) == 0)
